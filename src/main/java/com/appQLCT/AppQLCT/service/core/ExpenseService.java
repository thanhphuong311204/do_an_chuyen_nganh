...
public class ExpenseService {
        ...
        public Expense createExpense(ExpenseRequest request, User user) {
                ...
                Wallet wallet = walletRepository.findById(request.getWalletId())
                                .orElseThrow(() -> new RuntimeException(
                                                "Không tìm thấy ví ID: " + request.getWalletId()));
                ...
                User updatedUser = userStreakService.updateStreak(user, LocalDate.now());

                String coachMessage = coachMessageService.buildMessage(
                                updatedUser.getCurrentStreak());

                notificationService.createNotification(
                                user,
                                "Huấn luyện viên tài chính 🧠",
                                coachMessage,
                                "coach");
        ...
        public double getSpentInWeek(User user, int month, int week) {
                ...
                switch (week) {
                        ...
                        case 2 -> {
                                w_start = w1_end.plusDays(1);
                                w_end = w2_end;
                        }
                        case 3 -> {
                                w_start = w2_end.plusDays(1);
                                w_end = w3_end;
                        }
                        case 4 -> {
                                w_start = w3_end.plusDays(1);
                                w_end = w4_end;
                        }
                        default -> {
                                return 0;
